...
from diffnc.vendors.iosxe import PARSER


def test_parse_top_level() -> None:
    tree = PARSER.parse("hostname CSR-A\nplatform console serial\n")
...
def test_parse_indented_section_three_tuple_iface() -> None:
    text = (
    ...
    assert iface.line == "interface GigabitEthernet0/0/0"
...
def test_shutdown_toggle_collapses() -> None:
    text = "interface GigabitEthernet0/0/0\n shutdown\n no shutdown\n"
    tree = PARSER.parse(text)
    iface = tree.root.children[0]
