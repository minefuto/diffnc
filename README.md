...
* `group_sort_key(line) -> str | None` (optional; defaults to `None`, which disables clustering. Return a key — typically the line with its leading operation keyword stripped — to make order-insensitive children sharing that key display adjacently. Junos set form uses it so a path's `set` / `delete` / `activate` / `deactivate` operations cluster together.
